...
const ProfileC = () => {
  ...
  useEffect(() => {
    const userCoordinateur = sessionStorage.getItem("Coordinateur");
    console.log(
      "Retrieved Coordinateur from sessionStorage:",
      userCoordinateur
    ); // Log the retrieved data
    if (userCoordinateur) {
      setCoordinateur(JSON.parse(userCoordinateur)); // Assuming the sessionStorage holds a JSON string
    } else {
      // If no data found in sessionStorage, you can set default values or show a message
      console.log("No Coordinateur found in sessionStorage.");
    }
  }, []);

  const handleChange = (e) => {
    const { name, value } = e.target;
    setCoordinateur((prevState) => ({
      ...prevState,
      [name]: value,
    }));
  };
  ...
  const handleSave = () => {
    const { email, id, nom, prenom, telephone, password } = coordinateur;
    console.log(password);
    console.log(telephone);

    fetch(
      `http://localhost:8080/GestionWEB/coordinateur/update/${id}/${nom}/${prenom}/${email}/${password}/${telephone}`,
      {
        method: "PUT",
      }
    )
  ...
  const handleDelete = () => {
    const { id } = coordinateur;
    // Confirm before deleting
    const confirmation = window.confirm(
      "Are you sure you want to delete your account?"
    );
    if (confirmation) {
      fetch(`http://localhost:8080/GestionWEB/coordinateur/delete/${id}`, {
        method: "DELETE",
      })
        .then((response) => {
          if (response.ok) {
            ...
            sessionStorage.removeItem("Coordinateur"); // Clear session storage
  ...
  return (
    <div>
      ...
      <ul>
        ...
        <li>
          <strong>Prenom:</strong> {coordinateur.prenom}
        </li>
        <li>
          <strong>Email:</strong> {coordinateur.email}
        </li>
        <li>
          <strong>Telephone:</strong> {coordinateur.telephone}
        </li>
        <li>
          <strong>Mot de passe:</strong> {coordinateur.password}
        </li>
      </ul>
      <Button
        variant="contained"
